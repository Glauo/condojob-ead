...
export function EditAtividadeModal({ atividadeId }: { atividadeId: string }) {
  const router = useRouter();
  const [open, setOpen] = useState(false);
  ...
  const [form, setForm] = useState({
    titulo: "", tipo: "multipla_escolha",
    questao_texto: "", alternativa_a: "", alternativa_b: "", alternativa_c: "", alternativa_d: "",
    resposta_correta: "0",
  });
  ...
  async function abrir() {
    ...
    setLoading(true);
    try {
      const res = await fetch(`/api/atividades?id=${atividadeId}`);
      if (!res.ok) { setErro("Erro ao carregar atividade."); return; }
      const data = await res.json();
      const q = Array.isArray(data.questoes) ? data.questoes[0] : null;
      setForm({
        titulo: data.titulo || "",
        ...
        resposta_correta: String(q?.resposta_correta ?? 0),
      });
    } finally {
      setLoading(false);
    }
  ...
  async function salvar() {
    if (!form.titulo.trim() || !form.questao_texto.trim()) { setErro("Título e enunciado são obrigatórios."); return; }
    const questoes = [{
      texto: form.questao_texto,
      alternativas: form.tipo === "multipla_escolha"
        ? [form.alternativa_a, form.alternativa_b, form.alternativa_c, form.alternativa_d].filter(Boolean)
        : form.tipo === "verdadeiro_falso" ? ["Verdadeiro", "Falso"] : undefined,
      resposta_correta: (form.tipo === "multipla_escolha" || form.tipo === "verdadeiro_falso") ? Number(form.resposta_correta) : undefined,
    }];
    setSaving(true);
    const res = await fetch(`/api/atividades?id=${atividadeId}`, {
      method: "PUT",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ titulo: form.titulo, tipo: form.tipo, questoes }),
    });
    setSaving(false);
    if (!res.ok) { const d = await res.json().catch(() => ({})); setErro((d as { error?: string }).error || "Erro ao salvar."); return; }
    setOpen(false);
  ...
  return (
    <>
      <button className="btn btn-ghost btn-sm" onClick={abrir}>Editar</button>
      {open && (
        <div className="modal-overlay" onClick={(e) => e.target === e.currentTarget && setOpen(false)}>
          <div className="modal-box" style={{ maxWidth: "620px" }}>
            <div className="modal-header">
            ...
            {loading ? (
              <div className="modal-body" style={{ textAlign: "center", padding: "32px", color: "var(--cj-text-muted)" }}>Carregando…</div>
            ) : (
              <div className="modal-body">
                <div className="form-grid">
                  <div className="form-group form-group-full">
                  ...
                  <div className="form-group form-group-full">
                    <label className="form-label">Enunciado da questão *</label>
                    <textarea className="form-input form-textarea" rows={3} value={form.questao_texto} onChange={(e) => upd("questao_texto", e.target.value)} />
                  </div>
                  {form.tipo === "multipla_escolha" && (
                    <>
                      <div className="form-group form-group-full"><label className="form-label">Alternativa A *</label><input className="form-input" value={form.alternativa_a} onChange={(e) => upd("alternativa_a", e.target.value)} /></div>
            ...
            <div className="modal-footer">
              <button className="btn btn-secondary" onClick={() => setOpen(false)} disabled={saving}>Cancelar</button>
              <button className="btn btn-primary" onClick={salvar} disabled={saving || loading}>{saving ? "Salvando…" : "Salvar alterações"}</button>
            </div>
          </div>
        </div>
      )}
